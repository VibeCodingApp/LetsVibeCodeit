...
export function Ticker({ apps }: { apps: AppRow[] }) {
  ...
  const track = [...sample, ...sample].map(a => `<span class="flex items-center gap-1.5 px-5 whitespace-nowrap font-mono text-[13px] text-fg-2 border-r border-[var(--border)]"><span class="font-medium text-fg">${a.name}</span><span class="text-danger line-through opacity-70">$${a.priceMonthly}/mo</span></span>`).join('');
