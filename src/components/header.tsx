    ...
    <>
      ...
      <header data-site-header className="sticky top-0 z-50 bg-transparent">
        <div className="container-main site-header-inner glass px-5 flex items-center justify-between rounded-b-xl">
          ...
          <button onClick={toggle} aria-label={theme === 'dark' ? 'Switch to light mode' : 'Switch to dark mode'} aria-pressed={theme === 'dark'} className="ml-2 inline-flex h-9 w-9 shrink-0 cursor-pointer items-center justify-center rounded-full border border-[var(--border)] bg-surface-2/70 text-fg-2 transition-all duration-200 hover:border-[var(--border-2)] hover:text-fg hover:shadow-[0_0_10px_var(--primary-glow)]">
            {theme === 'dark'
              ? <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" aria-hidden="true"><circle cx="12" cy="12" r="4" /><path d="M12 2v2M12 20v2M4.93 4.93l1.41 1.41M17.66 17.66l1.41 1.41M2 12h2M20 12h2M4.93 19.07l1.41-1.41M17.66 6.34l1.41-1.41" /></svg>
        ...
        {open && (
          <nav className="md:hidden absolute top-[54px] left-0 right-0 bg-[var(--surface)] border-b border-[var(--border)] p-4 flex flex-col gap-0.5 z-50 shadow-[0_8px_24px_rgba(0,0,0,.5)]">
            ...
            <button onClick={() => { toggle(); setOpen(false); }} aria-label={theme === 'dark' ? 'Switch to light mode' : 'Switch to dark mode'} className="mt-1 inline-flex cursor-pointer items-center gap-2 self-start rounded-full border border-[var(--border)] bg-surface-2/70 px-3 py-2 font-mono text-sm text-fg-2 transition-colors hover:border-[var(--border-2)] hover:text-fg">
