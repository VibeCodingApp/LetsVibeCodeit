...
const labels: Array<{ key: keyof PostHogStats; label: string }> = [
  ...
  { key: 'viewsToday', label: 'Views Â· 24h' },
  { key: 'views7d', label: 'Views Â· 7d' },
  { key: 'visitors7d', label: 'Visitors Â· 7d' },
];

function formatValue(value: number | undefined) {
  return value === undefined ? 'â€”' : value.toLocaleString();
}
  ...
  return (
    <>
      ...
      <div className="bg-surface-2 border border-[var(--border)] rounded-xl p-6 text-center text-muted text-sm">
        {!response && 'Loading live PostHog dataâ€¦'}
