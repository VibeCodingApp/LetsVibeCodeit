...
function Card({ labelled, slotId, sponsor }: { labelled: boolean; slotId: string; sponsor?: SponsorPlacement }) {
  ...
  if (!sponsor) return <SponsorPurchaseButton plan="rail" slotId={slotId} labelled={labelled} ariaLabel={`Buy fixed rail slot ${slotId}`} className="mx-2 inline-flex shrink-0 items-center gap-2.5 rounded-full border border-[var(--border)] bg-[var(--surface-2)] px-4 py-1.5 font-normal transition-colors hover:border-[var(--border-2)]"><span className="rounded-full border border-[var(--border)] bg-[var(--surface-3)] px-2 py-0.5 font-mono text-[10px] uppercase tracking-[0.1em] text-muted-2">Ad</span><span className="whitespace-nowrap font-mono text-[12px] text-fg-2">promote your product here</span><span className="whitespace-nowrap font-mono text-[12px] font-bold text-primary">$199</span><span className="whitespace-nowrap font-mono text-[11px] text-muted-2">/30 days</span></SponsorPurchaseButton>;
  return (
    <a href={href} target={sponsor ? '_blank' : undefined} rel={sponsor ? 'sponsored noopener noreferrer' : undefined} aria-hidden={!labelled} tabIndex={labelled ? 0 : -1} className="mx-2 inline-flex shrink-0 items-center gap-2.5 rounded-full border border-[var(--border)] bg-[var(--surface-2)] px-4 py-1.5 no-underline transition-colors hover:border-[var(--border-2)]">
      {sponsor ? <img src={sponsor.creativeMode === 'banner' ? sponsor.bannerUrl : sponsor.iconUrl} alt="" className="h-5 w-8 rounded-md object-cover" /> : <span className="rounded-full border border-[var(--border)] bg-[var(--surface-3)] px-2 py-0.5 font-mono text-[10px] uppercase tracking-[0.1em] text-muted-2">Ad</span>}
